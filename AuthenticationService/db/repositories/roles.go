...
func (r *RoleRepositoryImpl) GetAllRoles() ([]*models.Role, error) {
	...
	// defer statement execute in LIFO order
	defer rows.Close()          // Ensure rows are closed after processing , defer will ensure that rows are closed after processing , this line will be executed after the function returns
...
func (r *RoleRepositoryImpl) CreateRole(name string, description string) (*models.Role, error) {
	...
	id, err := result.LastInsertId()  // Get the last inserted ID and name and description we already have
	if err != nil {
		return nil, err
	}
