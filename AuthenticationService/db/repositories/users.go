...
func (u *UserRepositoryImpl) GetByEmail(email string) (*models.User, error) {
	...
	err := row.Scan(&user.Id, &user.Email, &user.Password) // we have fetched the hashed password as well from the database
...
func (u *UserRepositoryImpl) Create(username string, email string, hashedPassword string) (*models.User, error) {
	...
	// populate the ? placeholders with the values supplied in the order they appear in the query
	result, err := u.db.Exec(query, username, email, hashedPassword)  // Exec executes a query that doesn't return rows, like INSERT, UPDATE, DELETE . 
	...
	lastInsertID, rowErr := result.LastInsertId()	 // Get the last inserted ID after the insert operation	
	if rowErr != nil {
...
func (u *UserRepositoryImpl) GetByID(id string) (*models.User, error) {
	...
	row := u.db.QueryRow(query, id) // id is the parameter for the query where ? is the placeholder

	// Step 3: Process the result
	user := &models.User{}   // this will be an empty user object 
	// Scan the row into the user object

	err := row.Scan(&user.Id, &user.Username, &user.Email, &user.CreatedAt, &user.UpdatedAt)  // sequence matters here, it should match the order in the query
