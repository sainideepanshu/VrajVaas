...
export async function generateRooms(jobData: RoomGenerationJob ) {
    ...
    const totalDays = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000*60*60*24));   // endDate.getTime() - startDate.getTime() will give us the difference in milliseconds (epoch), we divide it by 1000*60*60*24 to get the difference in days and then we take the ceil of it to get the total number of days
    ...
    while(currentDate < endDate) {
        ...
        if(batchEndDate > endDate ) {
            batchEndDate.setTime(endDate.getTime());   // if the batch end date is greater than the end date, we set it to the end date , this is avoiding the case where we try to create rooms for a date that is beyond the end date
        }
